...
import { uploadAvatar } from "@/utils/register";
export default {
  name: "CropperImage",
  components: {
    VueCropper,
  ...
  props: ["Name"],
  created () {
    var localImg = this.$http + JSON.parse(localStorage.getItem('userInfo')).avatar;
    this.option.img = localImg;
  },
  data() {
    return {
      name: this.Name,
        ...
        this.$refs.cropper.getCropBlob(async (data) => {
          ...
          uploadAvatar(formData).then((res) => {
            ...
            if (res.code === 200) {
              ...
              _this.$emit("uploadImgSuccess", res.imgUrl);
              _this.option.img = this.$http + res.imgUrl;
            } else {
              _this.$message({
