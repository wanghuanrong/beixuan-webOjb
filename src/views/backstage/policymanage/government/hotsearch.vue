<template>
  <div class="policydetails">
    <el-card>
      ...
      <h1 class="policytitle">{{policylist.year}}年{{ policylist.title }}<span> 解析</span></h1>
      ...
      <div :class="[showclass?'policyshuomin':'policyshuomin2']">
       <div class="two" style="width:25%;margin-right:7.5%;" v-html="policylist.applicationConditions"></div>
        <div class="two" style="width:25%;margin-right:7%;margin-left:1%" v-html="policylist.fundingStandards"></div>
        <div class="two towlast" style="width:25%;margin-left:1.5%" v-html="policylist.supplementaryExplanation"></div>
      </div>
    ...
    <el-card style="margin-top:20px">
      <h1 class="policytitle">
        {{policylist.year}}年{{ policylist.title }}<span class="yuanwen"> 原文</span>
      </h1>
      <div  class="contentxt" v-html="policylist.context"></div>
      <div v-show="policylist.context!='<p><br></p>'" class="clickurl" style="height:100px;">
    ...
    <!-- 第三个页面 -->
    <el-card style="margin-top:20px">
      <h1 class="policytitle">{{policylist.year}}年{{ policylist.title }} <span> 公示名单</span></h1>
        ...
        <div class="xialakuagn" v-if="piciobj2.length>=1">
         <el-dropdown trigger="click" @command="commandchange">
            <el-button type="primary">
              第{{keynum}}批<i class="el-icon-arrow-down el-icon--right"></i>
            </el-button>
            <el-dropdown-menu slot="dropdown" >
            <template>
              <div>
                   <el-dropdown-item  v-for="(item, index) in piciobj2" :command="composeValue(item.publicListId,item.batchNo)" :key="index">第{{ item.batchNo }}批</el-dropdown-item>
              </div>
            </template>
            </el-dropdown-menu>
          </el-dropdown>
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="companyName"  label="企业名称" width="260">
        </el-table-column>
        <el-table-column prop="departmentName" label="立项部门" align="center">
        </el-table-column>
        ...
        <el-table-column   label="立项金额" width="80" align="center">
          <template slot-scope="scope">
              <div v-show="scope.row.amount!=0">{{ scope.row.amount | moneyFormTwo}}万元</div>
   ...
   <div v-show="allpici.length==0" class="zhanwuimg">
    ...
    <!-- <el-card style="margin-top:20px">
      <div class="foottotal">
      <div class="foottitle">
        <img src="@/assets/personal/803.png" alt=""><span> 历史资助统计</span>
      </div>
      <div class="footmain">
      <div class="cardyear" v-for="(item,index) in cardlisd" :key="index">
        <p>{{item.year}}年</p>
        <div class="jinzi" style="margin-top:20px;">
           <div style="display:flex; justify-content: space-around;width:100%;padding: 0 50px;">
             <div>
              <span >{{item.totalPrice | money}}</span><i >万元</i> 
              </div>
              <div>
              <span>{{item.totalNum}}</span><i>家</i></div>
            </div>
           <div style="display:flex; justify-content: space-around;margin-top:20px;width:100%;padding: 0 50px;">
             <div>
             <img src="@/assets/indexbg/806.png" alt="" style="vertical-align: middle;"> <span class="zzjine"> 资助金额</span>
             </div>
             <div>
             <img  style="vertical-align: middle;" src="@/assets/indexbg/805.png" alt=""> <span class="zzjine" > 资助数量</span></div>
              </div>
        </div>
      </div>
      </div>
...
<script>
import { getpolicyId, getIdnoList, getsubsidylist,getyearTotal } from "@/utils/goverment.js";
  ...
  methods: {
     getpolicyInfo() {
      ...
      getpolicyId(this.id).then((res) => {
        ...
        console.log(this.policylist,"公司详情表");
      ...
      // 查询公司批次信息
       getIdnoList(this.id).then((res) => {
        ...
        console.log(this.allpici,"#222");
        // console.log(this.allpici,"公司批次详情");
      ...
      // 总补贴
      // getyearTotal(this.id).then(res=>{
      //   console.log(res,"年统计表");
    ...
    xminfo(){
      // console.log(url);
  if (this.piciurl === "未填写" || this.piciurl === null || this.piciurl === "") return this.$message.error("暂无链接");
      if(this.piciurl.indexOf("http") == -1 && this.piciurl.indexOf("www") == -1) return  this.$message.error("暂无链接")
      console.log(this.piciurl.indexOf("www"));
      if (this.piciurl.indexOf("www") != -1 && this.piciurl.indexOf("http") == -1) {
    ...
    newsinfo(){
     if (this.urll === "未填写" || this.urll === null || this.urll === "") return this.$message.error("暂无链接");
      if(this.urll.indexOf("http") == -1 && this.urll.indexOf("www") == -1) return  this.$message.error("暂无链接")
