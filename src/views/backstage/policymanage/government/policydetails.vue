  ...
  <div class="policydetails">
    <el-card>
      <div class="goback" @click="goback">
        <i class="iconfont iconfanhui1"></i> 返回
      </div>
      <h1 class="policytitle">
        {{ policylist.year }}年{{ policylist.title
        }}<span> 解{{ $store.state.clasname }}析</span>
      </h1>
      ...
      <div :class="[showclass ? 'policyshuomin' : 'policyshuomin2']">
        <div class="two" style="width:25%;margin-right:7.5%;">
          <div class="twoson" ref="diyg" v-html="oneinfo"></div>
        </div>
        <div
          class="two"
    ...
    <el-card style="margin-top:20px">
      <h1 class="policytitle">
        {{ policylist.year }}年{{ policylist.title }} <span> 公示名单</span>
      </h1>
    ...
    <uniInfo :uniid="uniId"></uniInfo>
    <!-- 第四个卡片 -->
    <el-card style="margin-top:20px">
      <div class="foottotal">
        <div class="foottitle">
          <img src="@/assets/personal/803.png" alt="" /><span>
            历史资助统计</span
          >
        </div>
        <div class="footmain">
          <div class="cardyear" v-for="(item, index) in cardlisd" :key="index">
            <p>{{ item.year }}<span style="font-size:11px;">年</span></p>
            <!-- <div class="jinzi" style="margin-top:20px;">
              <div
                style="display:flex; justify-content: space-around;width:100%;padding: 0 50px;"
              >
                <div class="onchild">
                
                   <span v-if="item.totalPrice!=0">{{ item.totalPrice}}</span
                  >
                  <span v-if="item.totalPrice==0" style="font-size:20px;">未公开</span>
                  <i v-if="!item.totalPrice==0">万</i>
                </div>
                <div>
                  <span v-if="item.totalNum!=0">{{ item.totalNum}}</span
                  >
                  <span v-if="item.totalNum==0" style="font-size:20px;">未公开</span>
                  <i v-if="!item.totalNum==0">家</i>
                </div>
              </div>
              <div
                style="display:flex; justify-content: space-around;margin-top:20px;width:100%;padding: 0 50px;"
              >
              <div></div>
                <div>
                  <img
                    src="@/assets/indexbg/806.png"
                    alt=""
                    style="vertical-align: middle;"
                  />
                  <span class="zzjine"> 资助金额</span>
                </div>
                <div>
                  <img
                    style="vertical-align: middle;"
                    src="@/assets/indexbg/805.png"
                    alt=""
                  />
                  <span class="zzjine"> 资助数量</span>
                </div>
              </div>
            </div> -->
    ...
    <!-- 尝试监听政策详情的高度 -->
    <!-- <div :class="[showclass ? 'policyshuomin2' : 'policyshuomin1']">
        <div  @click="handleHtml($event)"  class="two"   style="width:25%;margin-right:7.5%;">
          <div class="twoson" ref="diyg2" v-html="oneinfo" ></div>
        </div>
        <div
          class="two"
          style="width:25%;margin-right:7%;margin-left:1%"
...
export default {
  ...
  created() {
    ...
    this.uniId = this.$route.query.id;
    this.getpolicyInfo();
    this.gettab();
  },
  mounted() {
    ...
    // 监听高度失败
    // this.$refs.goback
    // // this.$refs.goback.scrollTop = 0
    //      const _this = this;
    // const erd = elementResizeDetectorMaker();
    // erd.listenTo(document.getElementById("newsinfos"), element => {
    //   _this.$nextTick(() => {
    //     console.log(_this.$refs.refnew.offsetHeight,"这是监听的数据");
    //     if(_this.$refs.refnew.offsetHeight>80){
    //       this.showbtn=true

    //     }
    //     // console.log(element.height,"高度");
    //     // _this.$refs.noteHeinewsinfosght.offsetHeight
    //   });
    // });
    // console.log(this.$refs['diyg2'].clientHeight, "总共的高");
    // this.handleHtml()
  },
  methods: {
    handleHtml($event) {
      console.log($event, "不知道啥东西");
    },
    getpolicyInfo() {
      // 查公司头部详情
    ...
    clikpc(id, index) {
      this.selected = index;
      this.$refs.namerefs.style = "border-bottom: 1px solid #0453FA;";
      console.log(id);
  ...
  filters: {
    ...
    moneyBt(val){
      console.log(String(val).length,"金额");
      let str=String(val)
      if(str.length>=7){
        // this.showMoney=true
...
.footmain {
  ...
  .cardyear {
    width: 20%;
    height: 230px;
    background: url("../../../../assets/indexbg/3449.png") no-repeat;
    background-size: cover;
    &:nth-child(2) {
      background: url("../../../../assets/indexbg/3450.png") no-repeat;
      background-size: cover;
    }
    &:nth-child(3) {
      background: url("../../../../assets/indexbg/3451.png") no-repeat;
      background-size: cover;
    }
    &:nth-child(4) {
      background: url("../../../../assets/indexbg/3452.png") no-repeat;
      background-size: cover;
    }
    &:nth-child(5) {
      background: url("../../../../assets/indexbg/5718.png") no-repeat;
      background-size: cover;
    }
    p {
      font-size: 26px;
